...
2. **Outlier detection and removal:** Use statistical methods like Z-score (remove data points beyond 3 standard deviations), IQR method (remove points below Q1-1.5×IQR or above Q3+1.5×IQR), or isolation forests to detect and remove outliers before training. However, be cautious as outliers might contain valuable information.
...
- Adds the sum of absolute values of coefficients: λΣ|wᵢ|
- Produces sparse models by driving some coefficients exactly to zero
- Performs implicit feature selection

**L2 Regularization (Ridge):**

- Adds the sum of squared coefficients: λΣwᵢ²
...
- Uses the sigmoid/logistic function: σ(z) = 1 / (1 + e^(-z)) to map linear combination of features to probability [0,1]
...
**Decision Boundary Interpretation:** The coefficients define a linear decision boundary in feature space. Each coefficient wᵢ tells you how much a one-unit increase in feature xᵢ changes the log-odds of the positive class. This makes logistic regression highly interpretable compared to black-box methods.
...
Where β coefficients are learned by minimizing Mean Squared Error (MSE) using ordinary least squares or gradient descent.
...
- **Linearity violated:** Model underfits; predictions are systematically biased. Consider polynomial features or non-linear models.
- **Independence violated:** Standard errors are underestimated; confidence intervals are too narrow. Use robust standard errors or GLS.
- **Normality violated:** Hypothesis tests and confidence intervals become unreliable (but predictions still work). Use robust methods or transformation.
- **Homoscedasticity violated:** Regression coefficients are unbiased but inefficient. Use weighted least squares or heteroscedasticity-robust standard errors.
- **Multicollinearity present:** Coefficients become unstable and hard to interpret. Use regularization (Ridge/Lasso) or remove correlated features.
...
- Time complexity: O(n×k×i×b) where b is batch size << n
...
- **MinMaxScaler:** Scale to [0,1] or [-1,1] - good when you want bounded features
...
- Example: L(ŷᵢ, yᵢ) = (ŷᵢ - yᵢ)² for one sample
...
- Example: J(w) = (1/n)Σⁿᵢ₌₁ L(ŷᵢ, yᵢ) + regularization
...
| Algorithm | Batch Gradient Descent | Stochastic GD (SGD) |
|-----------|------------------------|-------------------|
| Batch Size | Entire dataset | 1 sample |
| Updates per Epoch | 1 | n (dataset size) |
| Pros | Stable convergence, accurate gradient | Fast updates, escape local minima |
| Cons | Slow, high memory, can't use online | Noisy updates, erratic convergence |

| Mini-Batch Gradient Descent | Details |
|----------------------------|---------|
| Batch Size | Small subset (typically 32, 64, 128, 256) |
| Best of Both Worlds | Balance between gradient accuracy and update frequency |
| Advantages | GPU optimization, reduced variance, better generalization, most widely used in practice |
...
- Information Gain = H(parent) - Σ (|Sᵥ|/|S|) × H(Sᵥ)
...
- Higher χ² = stronger relationship = better split

**4. Variance Reduction (for Regression):**

- Used for continuous target variables
- Variance Reduction = Var(parent) - Σ (|Sᵥ|/|S|) × Var(Sᵥ)
...
**5. Context-Aware & Reinforcement Learning:** Incorporate time, location, device, or use RL to optimize long-term user engagement.
...
**5. F1 Scores:** Can compute micro-F1, macro-F1, or per-sample F1 (comparing predicted vs true label sets for each sample).
...
**Mathematical form:** (1 - Σφᵢ Lⁱ)(1-L)ᵈ yₜ = (1 + Σθᵢ Lⁱ)εₜ
